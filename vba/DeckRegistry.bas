...
' than escalating to `SaveAs path, 24` -- FIX-LIST P (2026-08-15 midday) had
' measured that SaveAs bricking a cloud deck read-only. That measurement
' predates the SAME DAY's afternoon fix to PropertyOnDisk's ByRef->ByVal bug:
' before the fix, reading the file (which every verifier does right before
' deciding whether to escalate) silently rewrote the caller's `path` from the
' true https:// URL to a local mapped path, so the SaveAs being measured
' targeted a DIFFERENT location than the document was open from -- which is
' what detaches and bricks it, not SaveAs-to-self itself.
'
' Re-measured 2026-08-16 with the fix in place, path genuinely unmangled:
' `pres.SaveAs path, 24` landed 5/5 in well under a second each, none flagged
' read-only afterward -- see vba/tools/onedrive_saveas_self_probe.ps1 and
' vba/tools/SaveAsSelfProbe.bas. All three verifiers now escalate to SaveAs on
' a cloud-hosted deck exactly as they already did on a local one; the
' cloud/local asymmetry these functions used to have is gone.
...
' ---------------------------------------------------------------------
' CLOUD-HOSTED SAVES NOW ESCALATE TO SaveAs-TO-SELF, SAME AS LOCAL ONES
' ---------------------------------------------------------------------
'
' Used to wait passively instead (up to 30s, in 5s steps) rather than ever
' calling `SaveAs path, 24` on a cloud-hosted deck -- see the declarations
' section above for why that used to be the rule and why it changed
' 2026-08-16. The wait-loop helpers this comment used to sit above
' (PauseSeconds, WaitForPropertyOnDisk, WaitForFileToMove) are gone with it;
' grep history if they're ever needed again, but they have no callers left in
' this module.
...
Public Function SaveDeckVerified(pres As Object) As String
    ...
    ' SELF. Cloud and local decks are treated identically now; see the
    ' declarations section for why the cloud path used to only wait.
...
Public Function SetDeckPeriodVerified(pres As Object, period As String, ByVal attempts As Long) As String
    ...
    For n = 1 To attempts
        ...
        ' Force the full rewrite, TO SELF -- cloud and local decks are treated
        ' identically now; see the declarations section for why the cloud path
        ' used to only wait instead.
        On Error Resume Next
