...
| `DELETIONS.md` | killed build scope, the question that killed it, what it would have cost | CURRENT — added 2026-08-16, append-only |
...
| `MILESTONE-PERCENTAGE-DESIGN.md` | the milestone-percentage feature (data model, the real `_PCT` shape, contiguous colour, position-gated visibility) | BUILT, LIVE-VERIFIED, RETROFITTED across all 46 real slides — added 2026-08-22 |
...
| `archive/*` | historical record, moved 2026-08-16 from repo root: `WORKPLAN.md`, `RUNSHEET.md`, `HANDOVER-Q4F26-DRAFTING.md`, `WORKED-EXAMPLE-STRATEGIC-ALIGNMENT.md`, `EXPECTED-TRACE-2026-08-14.md`, `PHASE1.md`, `NEXT-SESSION-2026-08-12.md`, `PROMPT_build.md`, `PROMPT_plan.md` | HISTORICAL — never edit to match today. Anything still-useful was migrated into `SYSTEM-OVERVIEW.md` or `CHECKLIST.md` with provenance before archiving. `PROMPT_build.md`/`PROMPT_plan.md` are a different kind of archive entry — not stale facts, but unused Ralph-autonomous-loop tooling, kept in case that mode is ever restarted for this project. `NEXT-SESSION-2026-08-12.md` surfaced a real architecture question before archiving — RESOLVED 2026-08-16, see `CHECKLIST.md`'s "ARCHITECTURE FORK" item and `archive/correspondence/`. |
| `archive/IMPLEMENTATION_PLAN.md`, `archive/FIRST-REAL-RUN.md`, `archive/CYCLE-FINDINGS-2026-07-30.md` | the design exchange and its findings, as they stood | HISTORICAL by design — moved to `archive/` 2026-08-16, checked not just filed. `IMPLEMENTATION_PLAN.md` had already self-migrated its live content to `AGENTS.md` (its own banner says so, verified). `CYCLE-FINDINGS` verified both its live fixes are still actually in the code. `FIRST-REAL-RUN.md` yielded two genuinely open items now on `CHECKLIST.md`: team distribution/multi-user (deliberately parked, unresolved) and no self-service single-field unmark. |
| `archive/specs/*` (was `specs/*`), `archive/vba-spike-notes/*` (was `vba/SPIKE_NOTES_*.md`) | the design exchange and its findings, as they stood | HISTORICAL by design — moved 2026-08-16. Sampled rather than read individually (3 of 47: the top-level `SPIKE_NOTES.md`, `specs/EXCHANGE-INDEX.md`, and a grep sweep of the largest file, `SPIKE_NOTES_BatchOnboardFlow.md`) — all three confirmed the genre and `EXCHANGE-INDEX.md` states its own resolution explicitly ("R13 must be built... everything else released," and R13 has been live and working all session). `check_docs.py`'s `HISTORICAL` regex fixed the same day: it exempted `^specs/` by anchored path, which broke silently the moment the directory moved — now exempts `^archive/` generally. |
