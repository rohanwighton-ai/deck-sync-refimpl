...
| 3 | Add a new slide type or derivative template | **UNBLOCKED for the colour-variant case (K/S/P) — the registration mechanism, verified from saved bytes, not reasoned about.** The old blocker (`TemplateSlide.FindTemplateFor` matching on type alone) is no longer what governs this: `DeckRegistry.RegisterTemplateLetter`/`LookupTemplateLetter`/`LookupTemplateForLetter` (registration), `TemplateSlide.ExistingTemplateForLetter` (the creation guard), `DeckRegistry.RegisterNewTemplateLetter` (claims a letter's own slot, and the type-level fallback only if nothing already holds it) — all built, all tested against real Office via COM, 2026-08-16. `Create template slide` is a real, repeatable toolbar button (was reachable only as a one-time post-onboarding prompt until the same day — found live when it was pressed and nothing happened). **K and S templates are now live, registered templates on the real deck**, not just the isolated test copy this row previously cited — but they carry a real field-propagation gap against the deck's full role set, which would let `SlideDuplication.DuplicateAndTag`'s own missing-field check report clean while shipping an incomplete slide. Current slide numbers and the size of that gap are fast-moving (already corrected twice in one evening) — see CHECKLIST.md's "Field propagation" section for the authoritative, current state rather than trusting a slide number or count restated here. **Not yet proven: end-to-end routing.** `RunSync.CreateMissingSlides` was given per-row letter-aware template selection (Scenario 3 step 3) and unit-tested with synthetic fixtures, but has never been run against a REAL register row that needs a K or S template — that is the next real test, not a formality: three fake rows (one per letter) into the register, then a real sync, is what actually proves a new K-project row gets cloned from the K template and not the P one. |
| 4 | Add a new field to an existing type | **MECHANISM PROVEN 2026-08-15, NOT FULLY PROPAGATED.** Tag once on the template -> propagate the role to every slide -> harvest the values — the machinery works, run by button. A full 43-slide census (CHECKLIST.md's "Field propagation" section, reached only after two earlier, too-small-sample counts on this exact row went stale within one evening) found the real gap is deck-wide, not confined to a couple of fields: most real project slides are missing several roles against the template's full set, and at least one field with real, already-collected data (`SAAFE_CASH`) is reaching zero real slides today. **Do not cite a field count here** — it has already gone stale twice tonight; CHECKLIST.md's "Field propagation" section is the authoritative, current source. The origin doc called this *"the largest stopped data flow in the project"*, and named the cause exactly — the machinery was good and the entry point was wrong; CHECKLIST.md's finding is that the same entry-point gap is wider than first reported. |
...
- **3, 4 and 7 are "next year"** — surviving a growing deck and a change of employer.
  **4's mechanism is proven but not fully propagated (see row 4 and CHECKLIST.md's "Field
  propagation" section); 7 is built and unrun; 3 is still blocked.**
...
**The OneDrive write-reliability risk is CLOSED, 2026-08-16 evening — proven on the
real add-in, repeated writes, independently cross-checked.** The original bricking bug
was a measurement artifact of the `ByRef` fix; the deeper limit found while re-proving
it (only a session's FIRST `CustomDocumentProperties` write ever lands on a cloud
deck) was real, and no retry/reopen technique could route around it. The actual fix
moved `DeckSyncPeriod`/`DeckSyncWorkbookPath`/`DeckSyncType`/`DeckSyncId` off
`CustomDocumentProperties` onto a hidden slide's shape content instead, which this
whole investigation had already shown syncs reliably. Rebuilt `addin109`, re-ran the
same probe that caught the original failure against the real, unmodified
`SetDeckPeriodVerified`: **8 for 8 repeated writes landed on one reused open cloud
file** — the exact scenario that failed 0/8 before. Full evidence, including two real
bugs found and fixed along the way (a `Variant`/`String` mistake and a test-script
false negative from VBA's `""` marshaling as PowerShell `$null`): `FIX-LIST.md` item
P's final update. This was the last blocker on Scenario 1.
