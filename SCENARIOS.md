...
| 3 | Add a new slide type or derivative template | **BLOCKED, and the reachability plan is now confirmed correct, not just plausible.** `TemplateSlide.FindTemplateFor` matches on slide type alone and `Exit Function`s on the first hit — real in the code, **latent, not currently firing**: the live deck has exactly **one** `is_template` slide (the green `P` one), so nothing collides yet. It fires the day a second template (K or S) is registered, which is also the day the feature is wanted. **Corrected 2026-08-16** — this row previously described three colour templates as already colliding; they don't exist yet. Confirmed by a second, independent read (`archive/correspondence/chat-to-code-2026-08-16-template-architecture-colour-vs-structure.md`) that the plan on `CHECKLIST.md` (per-letter registration, one type / three template-slide variants) does not conflict with the 12 Aug "one template, not three" ruling — that ruling is about slide **type**, this is about template-slide **variants** of one type. See `CHECKLIST.md`'s Scenario 3 section for the vocabulary and the condition on colour specifically. |
